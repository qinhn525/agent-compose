...
Microsandbox requires a reachable Docker daemon. It resolves the guest image, applies the pull policy, and exports the filesystem the base disk is built from through that daemon, and it never contacts a registry itself. Sandbox creation fails when the daemon is unavailable; there is no OCI-only fallback, so image authentication and pull configuration stay entirely on the Docker side.
